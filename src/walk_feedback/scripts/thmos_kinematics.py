...
pkg_path = rospack.get_path('thmos_zmp_walk')
lib_path = pkg_path + '/lib/libthmos_leg_ik.so'
lib = ctypes.CDLL(lib_path)

class THMOSLegIK:
    """inverse kinematics for two leg"""
    def __init__(self, way_left, way_right,leg_rod_length, motor_offset_left , motor_offset_right):
        """
        initialize class
        """
        self.leg_ang = [0] * 6
        self.end_pos = [0] * 6  
        self.motor_offset_left = motor_offset_left
        self.motor_offset_right = motor_offset_right
        # get C list
    ...
    def LegIKMove(self, LeftorRight, end_pos):
        ...
        if (LeftorRight == 'Left' or LeftorRight == 'left'):
            ...
            # write angle
            return [theta[0] + self.motor_offset_left[0] ,theta[1] + self.motor_offset_left[1] ,theta[2] + self.motor_offset_left[2] ,theta[3] + self.motor_offset_left[3] ,theta[4] + self.motor_offset_left[4] ,theta[5] + self.motor_offset_left[5] ]
        else:
            lib.leg_ik(self.c_leg_len, self.c_end_pos, self.c_motor_way_r, self.c_leg_ang)
            theta = ctypes.cast(self.c_leg_ang_ptr, ctypes.POINTER(ctypes.c_double * 6)).contents
            # write angle
            return [theta[0] + self.motor_offset_right[0] ,theta[1] + self.motor_offset_right[1] ,theta[2] + self.motor_offset_right[2] ,theta[3] + self.motor_offset_right[3] ,theta[4] + self.motor_offset_right[4] ,theta[5] + self.motor_offset_right[5] ]        
