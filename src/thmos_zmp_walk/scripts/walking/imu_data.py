...
class imu_Subscriber(object):
    ...
    def imu_callback(self, imu_msg: Imu):
        ...
        self.ax = (1.0 - filter) * imu_msg.linear_acceleration.x + filter * self.ax
        self.ay = (1.0 - filter) * imu_msg.linear_acceleration.y + filter * self.ay
        self.az = (1.0 - filter) * imu_msg.linear_acceleration.z + filter * self.az

        wx_new = (1.0 - filter) * imu_msg.angular_velocity.x + filter * self.wx
        wy_new = (1.0 - filter) * imu_msg.angular_velocity.y + filter * self.wy
        wz_new = (1.0 - filter) * imu_msg.angular_velocity.z + filter * self.wz
